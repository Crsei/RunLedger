...
>
> **快照推进（2026-09-18 补记）**：本文结论固定在目标快照 `0b2c501b`。此后 `30fd103`（websource 移植）使 `packages/utils` 的 `dom` 与 `turndown` 两族在 `src/websource/internal/` 下获得**部分**对等物，已在 §3 #14 行内标注；`packages/coding-agent` 侧的 `web/`、`exa/` 关闭见 [00](00-oh-my-pi-coding-agent-module-gap-report.md) 文首说明。复核当前状态请按 §9 命令重新生成事实。
