...
- 诚实边界(仍未完成):actor port 的真实 Session 命令面未接线,因此 `sendMessage`/`setModel`/`setActiveTools`/`exec` 等一律返回 `session_command_unavailable`(明确失败而非静默 no-op),`intent` 只记审计。
...
| P5 分发:安装/scope/激活 | **done**(actor 命令面除外) | 安装内核、activation 门禁、catalog/settings、cache/fetcher/manager、doctor、分发面接线、受治 git materialize、声明式回灌、host 资格判定、host 装配/工具准入调用点、`extension.host.inspect` 与 `plugin.config.*`(含 settings 键空间)均已交付;actor port 的真实 Session 命令面待做 |
