...
> 状态:**P5 已交付(含 host 装配,actor 命令面除外)**。P0–P5 已完成:P5 的分发内核、catalog/cache/fetcher/manager、settings schema、doctor、分发操作接线、受治 git materialize、声明式回灌、host 资格判定与 **extension host 装配(含工具准入调用点与 host 内工具 handler)** 均已落地;`plugin.config.*`(含 D13 settings 键空间)与 `extension.host.inspect` 已补;actor port 的真实 Session 命令面与 P6 CLI/TUI、P7 待做。
...
> 修订记录:2026-09-17 P6 TUI 动作入口收口。`/plugins` 支持内联参数:`install|upgrade <spec>` 与 `config <plugin-id> <key> <value>` 都走 `ExtensionConfirmModal`(确认后才 mutate,取消不开视图);安装/升级 scope 固定 `user`,notice 明说不启用/不信任/新 session 生效。
...
> 修订记录:2026-09-17 P7 watcher 生产接线收口。settings 新增 `plugins.watch`(默认关闭、user 层授权);生产组合按该开关构造 watcher 并用 `requestReload: async () => (await manager.reload()).status === "ready" ? "ready" : "pending"` 接既有 idle 语义;watcher 不可用经会话扩展审计上报。
> 修订记录:2026-09-17 P7 watcher Node 适配收口。新增 `src/storage/extensions/watch-adapter.ts`(唯一 `node:fs` 入口:root 上限整体拒绝、去重、stop 幂等关全部 watcher、坏 root 走错误通道不影响其余);`requestReload` 允许异步——生产接线必须 `await manager.reload()` 才知道是交换了还是被延到 idle 边界。
> 修订记录:2026-09-17 P7 可选文件 watcher 收口。新增 `src/extensions/plugins/reload-watcher.ts`:默认关闭、变更在 debounce 窗口内合并、只调用注入的 `requestReload()`(语义与 `ExtensionSnapshotStore.requestReload` 相同,idle 判定仍归既有 snapshot)、不 import `node:fs`(订阅是注入端口)、审计只记结果不记变更路径、timer `unref`。
> 修订记录:2026-09-17 P5 actor 命令面收口。生产组合新增晚绑定 `actorHost` 持有者(controller 在 extension composition 之后构建,由 `domain.ts` 填 `.current`),只暴露动作实际使用的四个方法;`sendMessage`/`setModel`/`setThinkingLevel` 从"一律拒绝"变为真实执行,其余动作仍**各自带原因**明确失败而不是静默 no-op。
> 修订记录:2026-09-17 P5 `plugin.config.*` 收口(含 D13 键空间)。settings 新增 `plugins.values`(packageId → setting → 值),非法条目**逐条**丢弃而非整体作废;read 返回每个已安装包的声明 schema + user 层生效值(补默认值并经 `resolvePluginSettings` 复核),write 先按声明校验再只合并被接受的键;CLI `plugin config [read|set <id> <key> <value>]` 以字符串传值、由 schema 定型。
> 修订记录:2026-09-17 P5 `extension.host.inspect` 收口。该 read 操作落地:bounded 只读投影(host 状态/generation/registryDigest/hostPid/激活时间/工具与订阅计数;失败时 code+message+retainedGeneration;每个候选的 scope/enabled/entrypoint 与声明工具计数/eligibility)。未装配 host 时报告 `disabled`+原因,而非空对象。
...
| `src/tui/interactive/extension-workflow.ts` + modal | 确认边界已交付(`ExtensionConfirmModal`),覆盖 trust/untrust 与 `/plugins install\|upgrade <spec>`、`/plugins config <plugin-id> <key> <value>` |
...
- 已完成(第九批,**`extension.host.inspect`**):`extension-composition.ts` 把该 read 操作加入 manifest 并接通 host 端口;投影只读且有界——host 状态(`idle|starting|ready|failed|stopped`)、`ready` 时的 generation/registryDigest/hostPid/activatedAt/工具与订阅计数、`failed` 时的 code+message+retainedGeneration、以及每个候选的 `packageId/version/scope/enabled/entrypoints 数/declaredTools 数/eligibility`(取自 P5 的 gate code)与账本 diagnostics;不暴露 entrypoint 之外的 native 路径。未装配 host 时返回 `{host:"disabled", reason}` 而不是空对象。证据:`tests/runtime/session-runtime/extension-distribution-domain.test.ts` 新增 2 用例;`tests/extensions/` 44 文件 341 用例通过。
...
- 已完成(第十批,**`plugin.config.*` + D13 键空间**):settings 新增 `plugins.values`,值只在 user 层授权(workspace 收窄规则由 `resolvePluginSettings` 执行);read/write 共用同一套 schema 校验;CLI 复合动词 `plugin config [read|set ...]` 的 mutation 由**子动作**决定。
- 已完成(第十一批,**actor 命令面**):生产组合新增晚绑定 `SessionExtensionActionHostHolder`,由 `domain.ts` 在 controller 就绪后填 `.current`,只暴露 `prompt`/`setThinkingLevel`/`getAvailableModels`/`selectModel` 四个方法(不把 controller 整体类型拉进 extensions 层)。`sendMessage` 以 `origin:"runtime"` 入 follow-up 队列——扩展可以注入消息但**不得冒充真实用户输入**;`setModel` 按 (providerId, modelId) 精确匹配可用模型,找不到返回 `model_unavailable` 而不是模糊匹配;`setThinkingLevel` 回传实际生效的档位。
- 仍未完成(逐项带原因,不是静默 no-op):`setActiveTools`(controller 没有 active-tool setter)、`setSessionName`(标题 mutation 未接该面)、`exec`(必须经 governed managed process)、`appendEntry`(无 session ledger 面,只记审计的键名)。**直接单测已补**(2026-09-17 第十二轮):actor port 的构造抽成可导出的 `createExtensionActionActorPort`,新增 `tests/runtime/session-runtime/extension-action-actor.test.ts` 6 用例覆盖三个真实动作、四个带原因的显式拒绝、intent 只记 digest,以及"持有者未绑定"路径。
...
- 已完成(TUI 动作入口,第十九批):`/plugins` 支持内联参数(`supportsInlineArgs` + `usage`),`/plugins install|upgrade <spec>` 与 `/plugins config <plugin-id> <key> <value>` 都先打开 `ExtensionConfirmModal`(`y`/Enter 确认、`n`/Esc 取消),确认后才经 attempt barrier 发 `plugin.install`/`plugin.upgrade`(scope `user`)/`plugin.config.write`;参数缺失或未知动词只给用法 notice、不开确认视图。安装/升级在 notice 里明说"不启用、不信任、新 session 才刷新声明式视图"。证据:`tests/tui/extension-selectors.test.ts` 新增 5 用例(install 确认前后、upgrade 取消、config 确认、缺参不开确认、notice),`tests/tui/commands/registry.test.ts` 固定 `plugins` 的内联参数位。
- 未完成:TUI/TTY 级人机验证(自动化覆盖了确认边界与 notice 的全部分支,但没有真实 PTY 里的按键证据);TUI 仍无 `marketplace add/remove` 入口(安装前需要先用 CLI 注册 marketplace)。
...
- 未完成:TUI/TTY 级人机验证仍缺真实按键证据(自动化覆盖了 notice 与确认边界的分支);TUI 仍无 marketplace add/remove 入口。
...
- 2026-09-17 actor port 直接单测:抽出 `createExtensionActionActorPort` 后用受控替身覆盖 6 条:消息以 `origin:"runtime"` 入队、`setModel` 精确匹配且不误选、`setThinkingLevel` 回传**实际**生效档位(含被夹到 `off` 的情形)、intent 只记 digest 不记正文、四个未接线动作各自带原因且 `appendEntry` 只审计键名、以及未绑定持有者时三个已接线动作均返回 `session_command_unavailable`。验证:`npx vitest run tests/extensions/ tests/runtime/session-runtime/extension-distribution-domain.test.ts tests/runtime/session-runtime/extension-action-actor.test.ts tests/cli/control-commands.test.ts` 46 文件 372 用例通过。
- 2026-09-17 可选文件 watcher:只交付协调器并把 fs 订阅留在注入端口,是因为扩展域禁止 raw fs(执行边界检查);`start()` 在未启用或无 root 时不订阅,`pending` 如实回传而不自行交换 generation(D11)。审计只记 outcome 不记变更路径,timer `unref` 以免可选加固延长 session 生命周期。验证:`npx vitest run tests/extensions/reload-watcher.test.ts` 8 用例;`tsc`(src 与 tests)、`check:execution-boundaries` 通过。
- 2026-09-17 watcher Node 适配:把唯一的 `node:fs` 入口收在 storage 适配层(root 上限**整体拒绝**而不是部分订阅、去重、stop 幂等、坏 root 走错误通道不拖垮其余、不自己做 debounce);`requestReload` 支持异步是必要的——生产接线必须 `await manager.reload()` 才知道交换是否发生,猜会让审计说反话。验证:`npx vitest run tests/extensions/reload-watcher.test.ts tests/extensions/watch-adapter.test.ts` 13 用例;全量 `npm run check` **exit 0**。
