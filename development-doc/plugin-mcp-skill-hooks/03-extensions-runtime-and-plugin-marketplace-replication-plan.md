...
| P5 分发:安装/scope/激活 | **done**(主机 supervisor 装配除外) | 安装内核、activation 门禁、catalog/settings、cache/fetcher/manager、doctor、分发面接线、受治 git materialize、声明式回灌与 host 资格判定均已交付(62 用例);`plugin.config.*`、`extension.host.inspect` 与 supervisor/准入/事件桥装配待做 |
