...
- 已完成(第十三批,**可选文件 watcher**):`reload-watcher.ts` + 8 条单测。默认关闭、`start()` 幂等且无 root 时不订阅、变更在窗口内合并成一次请求、`pending` 如实回传(不自行交换正在使用的 generation)、`stop()` 清订阅与 timer、审计只记 outcome、debounce 范围校验。Node 侧订阅实现留在 storage 适配层(扩展域禁 raw fs)。**尚未接线到生产组合**:Node 适配已就位,剩下的是 settings 开关(D13:user 层授权、默认关闭)与 composition 注入 `requestReload: async () => (await manager.reload()).status === "ready" ? "ready" : "pending"`。
...
| P7 加固与真实 smoke | **partial** | 失败语义矩阵(16 用例)、预算/上限矩阵(15 用例)、两份文档同步、真实 TTY 的安装→信任→启用→禁用→取消信任→卸载(九步 exit 0,`activation:"ready"`)、"调用"段缺口定位与收口(4 用例 + `skill.skills_root_empty` warning)已交付;文件 watcher 协调器(8 用例)与 Node 订阅适配(5 用例)已交付;settings 开关与 composition 注入、TUI 确认边界、`autoUpdate` 通知落点待做 |
