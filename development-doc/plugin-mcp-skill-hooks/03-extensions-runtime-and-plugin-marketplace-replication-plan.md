...
| P5 分发:安装/scope/激活 | **done**(actor 命令面除外) | 安装内核、activation 门禁、catalog/settings、cache/fetcher/manager、doctor、分发面接线、受治 git materialize、声明式回灌、host 资格判定、host 装配/工具准入调用点与 `extension.host.inspect` 均已交付(72 用例);actor port 的真实 Session 命令面与 `plugin.config.*` 待做 |
