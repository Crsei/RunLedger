...
- 仍未完成(逐项带原因,不是静默 no-op):`setActiveTools`(controller 没有 active-tool setter)、`setSessionName`(标题 mutation 未接该面)、`exec`(必须经 governed managed process)、`appendEntry`(无 session ledger 面,只记审计的键名)。**直接单元测试待补**:actor port 目前只被生产路径覆盖,计划在下一轮把它的构造抽成可导出工厂并补一组针对四个真实动作与四个显式拒绝的用例。
...
| P5 分发:安装/scope/激活 | **done** | 安装内核、activation 门禁、catalog/settings、cache/fetcher/manager、doctor、分发面接线、受治 git materialize、声明式回灌、host 资格判定、host 装配/工具准入调用点、`extension.host.inspect`、`plugin.config.*`(含 settings 键空间)与 actor 命令面(3 个真实动作 + 4 个带原因的显式拒绝)均已交付;actor port 的直接单测待补 |
