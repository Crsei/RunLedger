...
| P5 race/recovery/provider hardening | `partial` | 2026-08-17：已覆盖 editor/driver/selection/dispose 取消、stale completion、Agent provider error/abort、route manifest 缺失 fail-closed 与 receipt；2026-08-19：新增 router deny/auth missing/provider timeout 的 bounded typed diagnostic，沿 Session Owner TCP transient path 只通知 driver、不投影 text，durable replay 前后 digest 不变；新增独立 recovery barrier admission（即使 maintenance 误报 idle 也不 dispatch），crash-takeover barrier 零 provider request，以及真实 SessionRuntimeServer driver reconnect 的旧 completion 丢弃/新 epoch 接受回归；尚缺真实 auth/provider timeout 和真实模型 route evidence |
