...
- 剩余边界:runtime/Workspace external success 后、parent graph append 前 crash 仍不能 cold replay；`.launch-claim` 不是 authority-owned claim且没有 orphan/release reconciler；`launch_rejected` 仍走旧 not-started compensation；running child 的 composition shutdown 只能 fail closed；`stop_uncertain` 无 durable probe/operator resolution；Agent Workspace release receipt 未携带 WorktreeManager 的真实 release evidence；child controller/model/tool/Gateway/Sandbox 与 CLI/daemon/factory feature activation仍不存在。POSIX child 还可通过 `setsid`/double-fork 脱离 PGID，完整隔离需 Sandbox/cgroup/PID namespace；group-kill 的非 `ESRCH` 错误当前也会回退 direct PID，尚不能形成完整 tree-isolation authority evidence。production multi-agent继续保持 unsupported。
...
- 明确边界:`reasonEvidenceDigest` 在 shared terminal type/schema 中仍为 optional以读取既有事件；直接 `finish({ outcome: "stopped", reason: "cancelled" })`、terminal `interrupt("cancelled")` 与legacy optional terminal不由本切片强制提供独立 evidence。digest只是opaque bounded ref,不证明raw reason、actor authority、issuer/signature或外部证据可解析性。cold claim/reconciler、`launch_rejected` aggregate、`stop_uncertain` resolution、真实Workspace release evidence和child Agent loop仍未完成。
