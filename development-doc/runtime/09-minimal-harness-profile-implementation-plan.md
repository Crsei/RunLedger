...
后续入口与版本演进见 [Runtime 10：Agent Mode 入口实施计划](10-agent-mode-entry-implementation-plan.md)。2026-09-05 工作树新增 `minimal@2 = bash`、`plan@1`、schema 5、`/mode`/`--mode` 与用户 `agentMode`；本文上述 P1–P5 是历史验收快照，`minimal@1 = bash/edit` 与 immutable Session ref 保持不变。新版本自动化、built CLI 与人工门禁状态见 Runtime 10 实施记录。新版 receipt 在最终 child 工具注册后落盘，`descriptor-digests@1` 从 ordered descriptor table 自校验；历史 standard receipt 保留旧 raw-manifest 兼容边界。
