...
| 工具面扩展与呈现层 | [`plan/19 omp 工具面扩展计划`](plan/19-omp-tool-surface-expansion-plan.md) | 按 parity/02 §8 因果顺序：`read` 类型分派 + sqlite/archive 分支、`ask`、checkpoint/rewind、ast_grep·ast_edit、PDF，以及其后的逐工具 schema 预算（呈现层） | `in progress`；A1 实施中，B1–B3 待裁定（B2 需先撤销 Plan 16 裁定 4）；同时纠正 parity 00 对 read-pdf/sqlite-reader/rewind 的三处描述 |
