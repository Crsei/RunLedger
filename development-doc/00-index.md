...
| `/loop` 与 Goal Mode 适配 | [`plan/17-omp-loop-goal-mode-adaptation-plan.md`](plan/17-omp-loop-goal-mode-adaptation-plan.md) | goal canonical state、预算、模型工具、Owner 续跑、loop 条件与迭代、TUI；继承 02 的 D1–D5 范式 | `in progress`；P0.5 与 P0–P6 主体已提交（`12d0552`），D13/D10 已裁定且条件端口已接线；§13 为阶段证据，§14 为 goal 首轮/持久暂停、loop reset、动态描述与收尾验收清单 |
