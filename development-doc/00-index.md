...
| TUI / Composer Shape | [`tui/26-composer-shape-switch-plan.md`](tui/26-composer-shape-switch-plan.md) | 7 个 composer shape、共享 frame projector、用户级 `composer.shape`、`/shape`/`/setup` 入口与 OpenTUI native adapter | P0 `source-backed / passed`；P1–P4 source-aligned presentation adaptation；P5 `partial / pending-human`；P6 bounded trusted seam、P7 single-scene setup wizard 已实现；人工验收仍不标记 `implemented/accepted` |
