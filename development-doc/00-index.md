...
| Session Owner Runtime / Terminal | [`runtime/06-session-owner-runtime-replacement-plan.md`](runtime/06-session-owner-runtime-replacement-plan.md) | 用 session-scoped embedded runtime、SQLite ownership/durable state 与 localhost TCP 替代 machine/workspace Host；offline migration、recovery barrier、attachment lifetime、connection-scoped driver 与 checkpoint cache 已冻结；R0 contract freeze 与 R1 SQLite foundation 已完成（`session-owner/{types,schemas}.ts`、`session-server/protocol.ts`、`check:session-owner-boundaries`、[consumer/delete inventory](runtime/06-session-owner-inventory.md)、`storage/session-store/` database/schema/schema-compatibility/platform-capability） | 当前实现仍查 `runtime/05`、当前代码/tests；替代实施与完成状态查 `runtime/06` |
