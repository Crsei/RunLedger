...
| `/loop` 与 Goal Mode 适配 | [`plan/17-omp-loop-goal-mode-adaptation-plan.md`](plan/17-omp-loop-goal-mode-adaptation-plan.md) | oh-my-pi `3b3a6dc9bb` 的 loop/goal 移植：goal canonical state 与预算记账、`goal` 模型工具与准入、mode fragment 注入、loop runtime 迭代与审计、TUI 命令与 footer；继承 02 的 D1–D5 范式 | `planned`；逐文件比对与 P0–P6 阶段见该文；7 项待裁定见其 §11，D13/D10 阻塞 P5 与部分改动 |
...
| Extensions 运行时 / Plugin 分发与 Marketplace | [`plugin-mcp-skill-hooks/03-extensions-runtime-and-plugin-marketplace-replication-plan.md`](plugin-mcp-skill-hooks/03-extensions-runtime-and-plugin-marketplace-replication-plan.md) | oh-my-pi `3b3a6dc9bb` 的可执行扩展运行时(ExtensionAPI/事件桥/工具准入)与插件安装、scope、Claude 兼容 marketplace 的复刻设计与阶段 | **P0–P5 done、P6/P7 partial**：分发型运行时(host 装配、工具准入、事件桥、动作命令面)、分发/marketplace/doctor/settings 键空间、CLI 词表与真实闭环、失败与预算矩阵、真实 TTY 九步闭环、可选文件 watcher 均已交付；剩余 TUI 确认边界/通知落点、`plugin features`、descriptor 命名；阶段证据见该文 §15，总状态仍归 `01` |
