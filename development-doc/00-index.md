...
| Minimal Harness Profile | [`runtime/09-minimal-harness-profile-implementation-plan.md`](runtime/09-minimal-harness-profile-implementation-plan.md) | 会话级 immutable `standard@1` / `minimal@1`；minimal 固定完整 prompt、模型表面仅 governed `bash`/`edit`，不改变 Permission/Sandbox authority | P0–P4 已完成：contract、V4 durable identity/atomic fork、profile-aware production composition、创建期选择/只读显示与 authority-first recovery/audit；P5 最终生产验收与文档收口仍在推进 |
