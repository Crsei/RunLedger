...
| Minimal Harness Profile | [`runtime/09-minimal-harness-profile-implementation-plan.md`](runtime/09-minimal-harness-profile-implementation-plan.md) | 会话级 immutable `standard@1` / `minimal@1`；minimal 固定完整 prompt、模型表面仅 governed `bash`/`edit`，不改变 Permission/Sandbox authority | P0 contract、P1 V4 durable identity/atomic fork、P2 profile-aware production composition、P3 CLI/Session Domain/TUI 创建期选择与只读显示已完成；recovery/audit 与最终验收仍按 P4–P5 推进 |
