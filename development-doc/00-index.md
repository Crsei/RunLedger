...
| Plan Mode 完整度对齐 | [`plan-compact-memory/02-plan-mode-parity-implementation-plan.md`](plan-compact-memory/02-plan-mode-parity-implementation-plan.md) | 以 oh-my-pi `3b3a6dc9bb` 完成度为口径的 Plan Mode 行为交付：会话内进入/退出、mode/计划注入与压缩后重注入、模型侧进入与请求审批、审批决策集与 review 界面、实施交接、产物导出与 reentry | P0–P7 待实施；Plan Mode 行为交付的复选框状态入口以 02 为唯一来源（01 §0.7） |
