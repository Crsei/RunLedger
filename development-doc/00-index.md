...
| Storage / CLI | [`storage-cli/02-user-home-migration-handoff.md`](storage-cli/02-user-home-migration-handoff.md)、[`storage-cli/01-project-layout-cli-plan.md`](storage-cli/01-project-layout-cli-plan.md) | 用户级单一 home 迁移 handoff;旧项目级 `.runledger/`、settings、session 与 CLI 计划作为现行/历史输入 | 迁移状态查 `02`;现行旧布局见 [`project-cli-layout.md`](project-cli-layout.md) |
