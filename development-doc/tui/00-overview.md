...
`/dump` 命令、`session.request.inspect` 固定快照分页、provider 输入观测、OSC 52 原文剪贴板出口与 `runledger dump` headless 对应物只以 `28-system-prompt-dump-plan.md` 为权威；它不替换 20（命令注册表/派发/popup）、24/26（transcript 投影与摘要）、17/18（renderer 与流式）、27（主题）的任何 authority，也不改变 provider wire 工具 schema 的编码归属（`src/api/**`）。
