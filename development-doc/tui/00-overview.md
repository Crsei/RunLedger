...
> 可配置主体色槽与思考灰色的新实施入口：[Plan 27](27-configurable-ui-theme-and-thinking-color-plan.md)（implemented，验收证据见计划 §8）。已支持用户级 `uiTheme`、三套内置预设与色槽覆盖；下文保留历史设计，不代表当前 OpenTUI 实现。
...
Agent 读取、搜索和列举项目时的主时间线摘要、相邻探索归组及 Ctrl+T 详情分流只以 `26-codex-exploration-output-summary-plan.md` 为权威。当前实现为 `partial`：自动门禁和隔离标准 PATH TTY smoke 已通过，但不替代真实探索调用、dark/light 与选择复制的人类验收；Plan 24 已验收的 shell exec 截断也不能作为 read/search/list 正文压缩的完成证据。

`/dump` 命令、`session.prompt.inspect` 只读 operation、组装结果运行时捕获点、OSC 52 剪贴板出口与 `runledger dump` headless 对应物只以 `28-system-prompt-dump-plan.md` 为权威；它不替换 20（命令注册表/派发/popup）、24/26（transcript 投影与摘要）、17/18（renderer 与流式）、27（主题）的任何 authority，也不改变 provider wire 工具 schema 的编码归属（`src/api/**`）。
