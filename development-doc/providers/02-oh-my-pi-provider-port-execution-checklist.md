...
- 静态基线退役（同一改动的后半）：`scripts/ported-provider-catalog.ts` 的 `PortedProviderConfig` 增加 `retiredModels`，同时从 vendored 快照行与其它来源行（models.dev / 冻结输入）中剔除该 id——只加 `filterModel` 不够，被过滤掉的 id 会从另一来源复活。opencode-go 登记 `retiredModels: ["ox-alpha-free"]`，用于离线/刷新失败时不再显示已下线模型；provider 若实际仍提供，运行期权威刷新会把它带回来。
- 生成与复现：先用已提交 catalog 构造 frozen input 验证 generator 可离线逐字节复现（70/70 data 文件、shard 样本、aggregator 全部一致），再以同一 frozen input 重新生成——diff 恰为 `src/providers/data/opencode-go.json` 少一个 id，shard 与 aggregator 不变（shard 已与模型数量无关）。总模型数 4711 → 4710。
...
- 修改：`opencode-go.ts` 增加基于 `/models` 的动态发现（bearer 认证、调用方 signal、失败保留 last-known-good、空/坏 payload 与 HTTP 错误均 throw）。已知 id 沿用已审核 catalog 的 api/baseUrl/compat/limits，不重新推导；端点新增 id 按 models.dev npm 规则映射 wire，缺元数据时用与其它动态 provider 相同的保守默认（128k/8k），不臆测更大窗口。运行期落地语义（权威目录 vs overlay）与静态基线退役见上一节。
