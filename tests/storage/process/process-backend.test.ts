...
describe("R6 governed pipe process backend", () => {
	it("spawns once behind the audited barrier, streams output privately, and waits without exposing a PID", async () => {
		...
		try {
			...
			const retried = await backend.spawn({ handle, request: value, spawnClaimDigest: digest("claim"), constraintSnapshot: decision.snapshot });
	...
	it("settles a POSIX process group without claiming tree cleanup for none mode", async () => {
		...
		try {
			...
			const spawned = await backend.spawn({ handle, request: value, spawnClaimDigest: digest("claim"), constraintSnapshot: decision.snapshot });
	...
	it("kills a supervisor child and its descendant without a false zero-members claim", async () => {
		...
		try {
			...
			if (!decision.ok) throw new Error("supervisor decision failed");
			const handle = { ...decision.snapshot, revision: 0, requestDigest: value.requestDigest };
			const spawned = await backend.spawn({ handle, request: value, spawnClaimDigest: digest("descendant-claim"), constraintSnapshot: decision.snapshot });
	...
	it("stops the root when the durable output budget is exceeded and records the real kill", async () => {
		...
		try {
			...
			if (!decision.ok) throw new Error("decision failed");
			const handle = { ...decision.snapshot, revision: 0, requestDigest: value.requestDigest };
			const spawned = await backend.spawn({ handle, request: value, spawnClaimDigest: digest("limit-claim"), constraintSnapshot: decision.snapshot });
			const terminal = await spawned.process.wait(2_000);
			expect(terminal.outcome).toBe("killed");
	...
	it("enforces a request duration limit through the backend and settles as timed_out", async () => {
		...
		try {
			...
			if (!decision.ok) throw new Error("decision failed");
			const handle = { ...decision.snapshot, revision: 0, requestDigest: value.requestDigest };
		const spawned = await backend.spawn({ handle, request: value, spawnClaimDigest: digest("duration-claim"), constraintSnapshot: decision.snapshot });
			const terminal = await spawned.process.wait(2_000);
	...
	it("does not fabricate a terminal success when sealing durable output fails", async () => {
		...
		try {
			...
			if (!decision.ok) throw new Error("decision failed");
			const handle = { ...decision.snapshot, revision: 0, requestDigest: value.requestDigest };
			const spawned = await backend.spawn({ handle, request: value, spawnClaimDigest: digest("seal-failure-claim"), constraintSnapshot: decision.snapshot });
			const terminal = await spawned.process.wait(2_000);
