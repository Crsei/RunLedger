...
describe("SlashCommandPopup 过滤", () => {
  ...
  it("非空过滤:exact 匹配在前,prefix 匹配在后,保持注册表顺序", () => {
    ...
    popup.setFilter("/s");
    expect(popup.getVisibleRows().map((row) => row.name)).toEqual(["sessions", "settings", "skills", "skillsproviders", "scrollbar"]);
    expect(names(popup)).toEqual(["resume", "settings", "skills", "skillsproviders", "scrollbar"]);
    popup.setFilter("/c");
