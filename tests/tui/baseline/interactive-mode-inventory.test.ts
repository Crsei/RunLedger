 ...
 * refs.status.* 与 compactDomainResult 移入 `interactive/{event-controller,plan-workflow}.ts`；
 * 本文件只 pin facade 仍持有的字段与 S7 迁移目标位置。
 */
...
/** S7 迁移后组件 mutation 的归属模块。 */
const migratedMutationSites: ReadonlyArray<{ readonly file: string; readonly site: string }> = [
...
describe("B0 InteractiveMode inventory characterization", () => {
  ...
  it("pins the component mutation sites to be replaced by timeline/reducer projections", () => {
    ...
    for (const { file, site } of migratedMutationSites) {
      expect(interactiveDir(file), `component mutation ${site} in ${file}`).toContain(site);
    }
  });
