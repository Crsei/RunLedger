...
- OTLP 导出由标准 `OTEL_*` env 驱动（D2，不进 settings；`recording.mode` 仍是本地审计记录唯一 authority）：`initTelemetryExport()` 幂等、`OTEL_SDK_DISABLED=true` 或无 endpoint no-op、只支持 `http/protobuf`、service.name 默认 `runledger`；trace/log/metric 三信号 + `AgentMetricRecorder`（token 直方图 / `pi.omp.agent.*` counter）+ run-summary log 事件（D4，无中央 logger sink）；turn 边界 + 30s unref 定时 + 退出 `shutdownTelemetryExport()` 三处 flush；flush/shutdown 用 `Promise.allSettled` 隔离 collector/exporter 拒绝，不使成功 turn 或进程关闭失败；
- SDK 版本按 oh-my-pi 冻结快照实际解析线固定：direct SDK `2.10.0`，exporter 与 `sdk-logs` `0.220.0`，exporter transitive SDK `2.9.0`；不按 catalog 的 `^2.9.0` semver 下界降级。
- 测试：`tests/runtime/telemetry/{otel,run-summary,compaction-telemetry,otel-export}.test.ts` 59 测试（InMemorySpanExporter + AsyncLocalStorageContextManager + mock streamFn + production composition）。四条真实 oneshot 生产链、trace/log/metric 三信号非空 protobuf wire、collector outage 的 flush/shutdown 隔离均有回归。状态入口为 `development-doc/telemetry/01-telemetry-port-plan.md`（含 env 契约与验收矩阵结果）；门禁 `npm run check` / `npm test` / `npm run build` 全绿。
