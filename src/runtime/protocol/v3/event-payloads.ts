...
import { ChangeProposalRefSchema } from "./change-proposal.ts";
import {
...
export const RUNTIME_EVENT_PAYLOAD_SCHEMAS = {
	...
	"executor.execution_recorded": Type.Union([
		exact({
			requestId: id("command"),
			executorId: id("resource"),
			executorKind: remoteExecutorKind,
	...
	"change_proposal.recorded": exact({
	...
	"human_gate.decided": Type.Union([
		...
		exact({
			...
			decisionReasonDigest: digest,
			receiptDigest: digest,
			organizationReceiptId: id("receipt"),
			organizationReceiptDigest: digest,
			managedPolicyReceiptId: id("receipt"),
			managedPolicyReceiptDigest: digest,
			serverScope: Type.Literal("control_plane"),
		}),
	]),
	...
	"telemetry.delivery_recorded": Type.Union([
		exact({
		...
		exact({
			...
			manifestDigest: digest,
			batchDigest: digest,
			streamId: id("eventStream"),
			fromSequence: revision,
			fromEventHash: digest,
			throughSequence: revision,
			throughEventHash: digest,
			sinkId: id("resource"),
			sinkIdentityDigest: digest,
			exporterId: id("resource"),
			exporterIdentityDigest: digest,
			attempt: positiveInteger,
			outcome: Type.Literal("sink_acknowledged"),
			terminalReceiptId: id("receipt"),
