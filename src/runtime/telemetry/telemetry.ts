 ...
 * cheap pass-throughs.
 *
 * 移植自 oh-my-pi `packages/agent/src/telemetry.ts`(06aecdd5),按计划决策:
 *   - D1 属性命名空间与 oh-my-pi 逐字一致;
 *   - D7 `const enum` → `as const` 对象(见 `./semconv.ts`);
 *   - D8 省略 `EventLoopKeepalive`(Node 事件循环不会在长 promise 上冻结 timers);
 *   - RunLedger 无独立 `completeSimple` 导出,`instrumentedCompleteSimple`
 *     的 `completeImpl` 必须由调用方传入(`Models.completeSimple` 闭包);
 *   - 类型字段适配:`Usage.reasoning`(pi `reasoningTokens`)、
 *     `AssistantMessage.ttftMs`(pi `ttft`)、无 `upstreamProvider` / `usage.server`。
 */
...
import { AgentRunCollector, type AgentRunCoverage, type AgentRunSummary, type ToolStatus } from "./run-collector.ts";
import { GenAIAttr, GenAIOperation, type GenAIOperationName, OpenAIAttr, PiGenAIAttr, PiGenAIAggregateAttr } from "./semconv.ts";
import type { AgentTool } from "../types.ts";

/** Default tracer name. Override via {@link AgentTelemetryConfig.tracerName}. */
export const DEFAULT_TRACER_NAME = "runledger";
...
export interface ChatUsageEvent {
	 ...
	 * lowercased (mirrors {@link ProviderResponse.headers}). `undefined`
...
function emitTelemetryWarning(telemetry: AgentTelemetry | undefined, warning: AgentTelemetryWarning): void {
	...
	if (!hook) {
		if (warning.error === undefined) console.warn(`[runledger] ${warning.message}`);
		else console.warn(`[runledger] ${warning.message}`, warning.error);
		return;
	}
	try {
		hook(warning);
	} catch (err) {
		console.warn("[runledger] onTelemetryWarning threw; swallowing:", err);
	}
}
 ...
 */
export function startInvokeAgentSpan(telemetry: AgentTelemetry | undefined, model: Model<Api>): Span | undefined {
...
function applyChatResponseAttributes(span: Span, message: AssistantMessage): void {
	...
	// RunLedger 的 AssistantMessage 无 `upstreamProvider` 字段(pi 有),属性不发。
	if (message.ttftMs != null) span.setAttribute(GenAIAttr.ResponseTimeToFirstChunk, message.ttftMs / 1000);
...
function applyUsageAttributes(span: Span, usage: Usage | undefined): void {
	...
	if (usage.reasoning != null) {
		span.setAttribute(GenAIAttr.UsageReasoningOutputTokens, usage.reasoning);
	}
	// RunLedger 的 Usage 无 `server` 字段(pi 有),server-side tool 计数不发。
}
 ...
 * {@link ProviderResponse.headers} produces.
 */
...
export interface InstrumentedChatSpanOptions {
	 ...
	 * `compaction_summary`, `auto_title`, `child_agent`, `gateway`. Free-form
	 * to allow callers outside this package to add new kinds without bumping
	 * the helper.
	 */
	...
	readonly attributes?: Attributes;
	/**
	 * The underlying completion implementation. RunLedger 没有独立
	 * `completeSimple` 导出(`Models.completeSimple` 是实例方法,不能脱离
	 * `this` 使用),因此本字段必填:调用方传入
	 * `(m, c, o) => models.completeSimple(m, c, o)` 闭包。
	 */
...
/**
 * Wrap a {@link Models.completeSimple} round-trip with the same chat-span
 * lifecycle the agent loop uses for streamed turns: `startChatSpan` → run inside
 * the active span → `finishChatSpan` on success, `failChatSpan` on throw.
 *
...
export async function instrumentedCompleteSimple<TApi extends Api>(
	...
	// D8:oh-my-pi 的 `EventLoopKeepalive` 是 Bun 专属(Node 事件循环不会在
	// 长 promise 上冻结 timers),RunLedger 运行在 Node,直接省略。
	...
	try {
		return await runInActiveSpan(chatSpan, async () => {
			const message = await span.completeImpl(model, ctx, {
				...options,
