 ...
 *
 * 移植自 oh-my-pi `packages/agent/src/run-collector.ts`(06aecdd5)。
 */

import type { Api, AssistantMessage, Model, StopReason } from "../../types.ts";
...
export class AgentRunCollector {
	...
	beginChat(
		span: Span,
		init: { readonly stepNumber: number; readonly model: Model<Api>; readonly provider?: string },
	): void {
	...
	endChat(
		...
		const reasoningOutputTokens = usage?.reasoning ?? 0;
