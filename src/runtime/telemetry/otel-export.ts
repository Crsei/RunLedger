 ...
 *
 * RunLedger 的 agent core(`src/runtime/telemetry/telemetry.ts`)通过全局
 * `@opentelemetry/api` tracer 发出 GenAI span,并通过 run-level 回调暴露
 * metrics/log 管道。本模块在标准 `OTEL_*` endpoint env 设置时注册 OTLP/proto
 * trace/log/metric SDK provider,让 RunLedger 可被任意 OTLP collector 观测,
 * 无 vendor 耦合。
 *
 * 只支持 `http/protobuf` 传输:`OTEL_EXPORTER_OTLP*_PROTOCOL` 为 `grpc` 或
 * `http/json` 时对应信号禁用并 warn,绝不 misroute protobuf 载荷。依赖线锁定
 * oh-my-pi 验证线(D6);RunLedger 跑 Node(非 Bun),1.x OTLP 线的 Bun 死锁不适用,
 * 但保持一致便于对照复验。
 *
 * 移植自 oh-my-pi `packages/coding-agent/src/telemetry-export.ts`(06aecdd5),按计划决策:
 *   - D4 日志信号裁剪:RunLedger 无中央 logger,只发结构化 run-summary 事件与
 *     telemetry warning(`OTEL_LOG_LEVEL` 过滤),不注册 log sink;
 *   - D5 service.name 默认 `runledger`(`OTEL_SERVICE_NAME` 仍可覆盖);
 *   - postmortem 钩子替换为 `shutdownTelemetryExport()` + 进程退出接缝。
 */
import type {
	AgentRunCoverage,
	AgentRunSummary,
	ToolStatus,
} from "./run-collector.ts";
import type {
...
/**
 * Periodic flush interval. A long-lived process (the Session Owner server is
...
const SERVICE_NAME = "runledger";

type TelemetrySignal = "trace" | "log" | "metric";
/** RunLedger 无中央 logger 模块,log 级别直接在本地裁剪。 */
type OtelLogLevel = "none" | "error" | "warn" | "info" | "debug";
...
const LOG_SEVERITY: Record<Exclude<OtelLogLevel, "none">, SeverityNumber> = {
...
const LOG_LEVEL_WEIGHT: Record<Exclude<OtelLogLevel, "none">, number> = {
	error: 0,
	warn: 1,
	info: 2,
	debug: 3,
...
async function registerProviders(signalConfig: SignalConfig): Promise<void> {
	...
	if (signalConfig.metric) {
		...
		metricRecorder = new AgentMetricRecorder(metrics.getMeter("runledger"));
	}
	...
	if (signalConfig.log) {
		...
		otelLogger = logProvider.getLogger("runledger");
		// D4:不注册中央 logger sink;只发结构化 run-summary 与 telemetry warning。
	}
...
/**
 * 进程退出接缝(对齐 pi postmortem):beforeExit + 信号处理器 best-effort 调
 * shutdown。仅在 provider 真正注册后挂接,幂等;不影响 CLI 自身的信号语义。
 */
...
/** 关闭三信号 provider(forceFlush + shutdown)。幂等;未注册时 no-op。 */
export async function shutdownTelemetryExport(): Promise<void> {
	...
	if (traceProvider) {
		shutdowns.push(traceProvider.shutdown());
		traceProvider = undefined;
	}
	if (logProvider) {
		shutdowns.push(logProvider.shutdown());
...
export function signalEnabled(
	...
	if (protocol && protocol !== "http/protobuf") {
		console.warn(
			`[runledger] OTEL ${signal} export disabled: OTEL_EXPORTER_OTLP_PROTOCOL=${protocol} is unsupported; supported=http/protobuf`,
		);
		return false;
	}
	return true;
...
export class AgentMetricRecorder {
...
function emitOtelLog(
	...
	if (!otelLogger || level === "none") return;
...
export function parseOtelLogLevel(raw: string | undefined): OtelLogLevel {
	if (!raw) return "info";
