...
const LM_STUDIO_SENTINEL_API_KEY = "lm-studio-local";
...
// 简化：未移植 lm-studio 原生探针(fetchLmStudioNativeModelMetadata)，仅用 {base}/models 兜底路径
async function fetchModels(
	...
	const apiKey = rawKey && rawKey !== LM_STUDIO_SENTINEL_API_KEY ? rawKey : undefined;
	...
	if (!response.ok) {
		throw new Error(`Could not load LM Studio models: ${response.status}: ${truncateBody(await response.text())}`);
	}

	const payload: unknown = await response.json();
	if (!isRecord(payload) || !Array.isArray(payload.data)) throw new Error("Invalid LM Studio model catalog response");
	...
	if (models.size === 0) throw new Error("LM Studio returned an empty model catalog");
	return [...models.values()];
}

const lmStudioAuth: ApiKeyAuth = {
	name: "LM Studio API key (optional)",
	login: async (interaction: AuthInteraction) => {
		const key = (await interaction.prompt({
			type: "secret",
			message: "Enter LM Studio API key (optional — empty for keyless local server)",
		})).trim();
		return key ? { type: "api_key", key } : { type: "api_key" };
	},
	resolve: async ({ ctx, credential }) => {
		const key = credential?.key ?? (await ctx.env("LM_STUDIO_API_KEY"));
		return {
			auth: { apiKey: key ?? LM_STUDIO_SENTINEL_API_KEY },
			source: key ? (credential?.key ? "stored credential" : "LM_STUDIO_API_KEY") : "no auth required",
		};
	},
};

export function lmStudioProvider(options: LmStudioProviderOptions = {}): Provider<"openai-completions"> {
	const baseUrl = normalizeBaseUrl(options.baseUrl ?? process.env.LM_STUDIO_BASE_URL, LM_STUDIO_DEFAULT_BASE_URL);
	const staticModels = Object.values(LM_STUDIO_MODELS).map((model) => ({ ...model, baseUrl }));
