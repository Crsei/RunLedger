...
const NANOGPT_DEFAULT_BASE_URL = "https://nano-gpt.com/api/v1";

// 剔除 :thinking 变体与非聊天(语音/转写/审核/实时/嵌入等)模型
...
function normalizeBaseUrl(value: string | undefined): string {
	const configured = value?.trim() || NANOGPT_DEFAULT_BASE_URL;
...
function isLikelyNanoGptTextModelId(id: string): boolean {
	...
	if (NANO_GPT_THINKING_SUFFIX_RE.test(normalized)) return false;
...
function mapModel(
	...
	if (!id || !isLikelyNanoGptTextModelId(id)) return undefined;
...
async function fetchModels(
	...
	if (!apiKey) throw new Error("NanoGPT API key is not configured");
	...
	if (!response.ok) {
		throw new Error(`Could not load NanoGPT models: ${response.status}: ${truncateBody(await response.text())}`);
	}

	const payload: unknown = await response.json();
	if (!isRecord(payload) || !Array.isArray(payload.data)) throw new Error("Invalid NanoGPT model catalog response");
	...
	if (models.size === 0) throw new Error("NanoGPT returned an empty model catalog");
	return [...models.values()];
}

export function nanogptProvider(options: NanogptProviderOptions = {}): Provider<"openai-completions"> {
	const baseUrl = normalizeBaseUrl(options.baseUrl ?? process.env.NANOGPT_BASE_URL);
	const staticModels = Object.values(NANOGPT_MODELS).map((model) => ({ ...model, baseUrl }));
