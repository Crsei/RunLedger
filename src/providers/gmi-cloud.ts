...
import { GMI_CLOUD_MODELS } from "./gmi-cloud.models.ts";

const GMI_CLOUD_DEFAULT_BASE_URL = "https://api.gmi-serving.com/v1";

export interface GmiCloudProviderOptions {
	baseUrl?: string;
	fetch?: typeof fetch;
}
...
function normalizeBaseUrl(value: string | undefined): string {
	const configured = value?.trim() || GMI_CLOUD_DEFAULT_BASE_URL;
...
async function fetchModels(
	...
	if (!apiKey) throw new Error("GMI Cloud API key is not configured");
	...
	if (!response.ok) {
		throw new Error(`Could not load GMI Cloud models: ${response.status}: ${truncateBody(await response.text())}`);
	}

	const payload: unknown = await response.json();
	if (!isRecord(payload) || !Array.isArray(payload.data)) throw new Error("Invalid GMI Cloud model catalog response");
	...
	if (models.size === 0) throw new Error("GMI Cloud returned an empty model catalog");
	return [...models.values()];
}

export function gmiCloudProvider(options: GmiCloudProviderOptions = {}): Provider<"openai-completions"> {
	const baseUrl = normalizeBaseUrl(options.baseUrl ?? process.env.GMI_BASE_URL);
	const staticModels = Object.values(GMI_CLOUD_MODELS).map((model) => ({ ...model, baseUrl }));
