...
import { SILICONFLOW_CN_MODELS } from "./siliconflow-cn.models.ts";

export interface SiliconflowCnProviderOptions {
	baseUrl?: string;
	fetch?: typeof fetch;
}
...
function normalizeBaseUrl(value: string | undefined): string {
	const configured = value?.trim() || "https://api.siliconflow.cn/v1";
...
function mapModel(
	...
	return {
		...(reference ?? {
			...
			provider: "siliconflow-cn" as const,
			baseUrl,
			reasoning: false,
...
async function fetchModels(
	...
	if (!apiKey) throw new Error("SiliconFlow (China) API key is not configured");
	...
	if (!response.ok) {
		throw new Error(`Could not load SiliconFlow (China) models: ${response.status}: ${truncateBody(await response.text())}`);
	}

	const payload: unknown = await response.json();
	if (!isRecord(payload) || !Array.isArray(payload.data)) {
		throw new Error("Invalid SiliconFlow (China) model catalog response");
	}
	...
	if (models.size === 0) throw new Error("SiliconFlow (China) returned an empty model catalog");
	return [...models.values()];
}

export function siliconflowCnProvider(options: SiliconflowCnProviderOptions = {}): Provider<"openai-completions"> {
	const baseUrl = normalizeBaseUrl(options.baseUrl ?? process.env.SILICONFLOW_CN_BASE_URL);
	const staticModels = Object.values(SILICONFLOW_CN_MODELS).map((model) => ({ ...model, baseUrl }));
	return createProvider({
		id: "siliconflow-cn",
		name: "SiliconFlow (China)",
		baseUrl,
		auth: { apiKey: envApiKeyAuth("SiliconFlow (China) API key", ["SILICONFLOW_CN_API_KEY"]) },
		models: staticModels,
