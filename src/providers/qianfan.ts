...
const QIANFAN_DEFAULT_BASE_URL = "https://qianfan.baidubce.com/v2";

export interface QianfanProviderOptions {
	baseUrl?: string;
	fetch?: typeof fetch;
}
...
function normalizeBaseUrl(value: string | undefined): string {
	const configured = value?.trim() || QIANFAN_DEFAULT_BASE_URL;
...
async function fetchModels(
	...
	if (!apiKey) throw new Error("Qianfan API key is not configured");
	...
	if (!response.ok) {
		throw new Error(`Could not load Qianfan models: ${response.status}: ${truncateBody(await response.text())}`);
	}

	const payload: unknown = await response.json();
	if (!isRecord(payload) || !Array.isArray(payload.data)) throw new Error("Invalid Qianfan model catalog response");
	...
	if (models.size === 0) throw new Error("Qianfan returned an empty model catalog");
	return [...models.values()];
}

export function qianfanProvider(options: QianfanProviderOptions = {}): Provider<"openai-completions"> {
	const baseUrl = normalizeBaseUrl(options.baseUrl ?? process.env.QIANFAN_BASE_URL);
	const staticModels = Object.values(QIANFAN_MODELS).map((model) => ({ ...model, baseUrl }));
