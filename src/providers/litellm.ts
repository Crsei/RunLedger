...
const LITELLM_SENTINEL_API_KEY = "litellm-local";
...
// 简化：未移植 litellm 管理端丰富元数据端点(/model_group/info 等)，仅用 {base}/models 兜底路径
async function fetchModels(
	...
	const apiKey = rawKey && rawKey !== LITELLM_SENTINEL_API_KEY ? rawKey : undefined;
	...
	if (!response.ok) {
		throw new Error(`Could not load LiteLLM models: ${response.status}: ${truncateBody(await response.text())}`);
	}

	const payload: unknown = await response.json();
	if (!isRecord(payload) || !Array.isArray(payload.data)) throw new Error("Invalid LiteLLM model catalog response");
	...
	if (models.size === 0) throw new Error("LiteLLM returned an empty model catalog");
	return [...models.values()];
}

const litellmAuth: ApiKeyAuth = {
	name: "LiteLLM API key (optional)",
	login: async (interaction: AuthInteraction) => {
		const key = (await interaction.prompt({
			type: "secret",
			message: "Enter LiteLLM API key (optional — empty for keyless local proxy)",
		})).trim();
		return key ? { type: "api_key", key } : { type: "api_key" };
	},
	resolve: async ({ ctx, credential }) => {
		const key = credential?.key ?? (await ctx.env("LITELLM_API_KEY"));
		return {
			auth: { apiKey: key ?? LITELLM_SENTINEL_API_KEY },
			source: key ? (credential?.key ? "stored credential" : "LITELLM_API_KEY") : "no auth required",
		};
	},
};

export function litellmProvider(options: LitellmProviderOptions = {}): Provider<"openai-completions"> {
	const baseUrl = normalizeBaseUrl(options.baseUrl ?? process.env.LITELLM_BASE_URL, LITELLM_DEFAULT_BASE_URL);
	const staticModels = Object.values(LITELLM_MODELS).map((model) => ({ ...model, baseUrl }));
	return createProvider({
		id: "litellm",
		name: "LiteLLM",
		baseUrl,
		auth: { apiKey: litellmAuth },
		models: staticModels,
