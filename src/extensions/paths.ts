...
export type ContainedPathResult =
	| { readonly ok: true; readonly root: string; readonly path: string; readonly relativePath: string }
	| { readonly ok: false; readonly code: "missing" | "escape" | "invalid"; readonly message: string };
...
export async function resolveContainedPath(
	...
	try {
		...
		const lexicalRelative = relative(canonicalRoot, lexical);
		let current = canonicalRoot;
		for (const component of lexicalRelative.split(sep).filter(Boolean)) {
			current = join(current, component);
			const entry = await storage.stat(current, { followSymlinks: false });
			if (!entry.ok) return { ok: false, code: "missing", message: entry.message };
			if (entry.value.kind === "symlink" || entry.value.kind === "other") {
				return { ok: false, code: "invalid", message: "resource path contains a link or device" };
			}
		}
...
export async function resolveDeclaredPath(
	storage: ExtensionStoragePort,
	root: string,
	declaration: string,
): Promise<ContainedPathResult> {
...
export async function findProjectBoundary(
	storage: ExtensionStoragePort,
	cwd: string,
...
export function sourceKey(source: ExtensionSource, canonicalRoot: string): string {
	const digest = createHash("sha256").update(canonicalRoot, "utf8").digest("hex").slice(0, 16);
...
export async function discoverExtensionRoots(args: {
	...
	const add = async (source: ExtensionSource, path: string, priority: number): Promise<void> => {
		try {
			...
			seen.add(canonical);
			const pluginManifest = await args.storage.stat(join(canonical, ".runledger-plugin", "plugin.json"));
			roots.push({
				source,
				sourceKey: sourceKey(source, canonical),
				rootPath: canonical,
				priority,
				...(pluginManifest.ok && pluginManifest.value.kind === "file" ? { layout: "plugin-root" as const } : {}),
			});
		} catch {
			// 可选 discovery root 不存在时只由调用方的总体诊断决定。
		}
	};
	...
	return roots.sort((left, right) => left.priority - right.priority || (left.rootPath < right.rootPath ? -1 : 1));
}
