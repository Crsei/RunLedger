...
/** Lock-scoped canonical read-modify-write used by all partial mutations. */
export async function updateProjectSettings(
	options: SettingsStoreOptions,
	mutate: (current: ProjectSettings) => ProjectSettingsInput,
): Promise<ProjectSettings> {
	const path = getSettingsPath(options);
	await fs.mkdir(dirname(path), SETTINGS_MKDIR_OPTS);
	const release = await lockfile.lock(path, {
	...
	try {
		...
		await saveProjectSettingsUnlocked(options, next);
		return await loadProjectSettings(options);
	} finally {
		await release();
	}
}
