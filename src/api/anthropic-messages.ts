/**
 * Anthropic Messages adapter —— 公共入口 facade。
 *
 * S8.3 拆分后实现位于 `anthropic-messages/`:
 * - `sse-decoder.ts`        跨 chunk 的 SSE line decoder;
 * - `event-mapper.ts`       SSE event 过滤/解析 + stop reason 映射;
 * - `client.ts`             client 构造 + auth/header 解析;
 * - `params.ts`             buildParams + cache control/effort 映射;
 * - `message-conversion.ts` message/tool_result 转换 + Claude Code tool 名映射;
 * - `tool-conversion.ts`    tool schema 转换 + fine-grained beta 判定;
 * - `types.ts`              公共选项/compat 类型。
 *
...
import { fromClaudeCodeName } from "./anthropic-messages/message-conversion.ts";
import { shouldUseFineGrainedToolStreamingBeta } from "./anthropic-messages/tool-conversion.ts";
import { buildParams, mapThinkingLevelToEffort, resolveCacheRetention } from "./anthropic-messages/params.ts";
import { assertRequestAuth, createClient } from "./anthropic-messages/client.ts";
import { iterateAnthropicEvents, mapStopReason } from "./anthropic-messages/event-mapper.ts";
import { buildCopilotDynamicHeaders, hasCopilotVisionInput } from "./github-copilot-headers.ts";

import type { AnthropicOptions } from "./anthropic-messages/types.ts";
export type {
	AnthropicOptions,
	AnthropicEffort,
	AnthropicThinkingDisplay,
} from "./anthropic-messages/types.ts";
