 ...
 * through the injected {@link WebSearchCredentialPort} (Bearer token), and
 * responses are categorized result buckets rather than the legacy flat object
 * array.
 */
...
export async function searchWithKagi(query: string, options: KagiSearchOptions): Promise<KagiSearchResult> {
	...
	const response = await withApiKey(
		options.credentials,
		"kagi",
		"Kagi credentials not found. Set KAGI_API_KEY or login with 'omp /login kagi'.",
		async apiKey => {
			const res = await options.fetch(KAGI_SEARCH_URL, {
				method: "POST",
				headers: {
