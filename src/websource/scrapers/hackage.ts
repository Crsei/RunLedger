import { compareVersions, tryParseJson } from "../internal/platform.ts";
...
export const handleHackage: SpecialHandler = async (
	...
	try {
		...
		const versionResult = await loadPage(context, versionUrl, {
			timeout,
		...
		const cabalResult = await loadPage(context, cabalUrl, {
			timeout,
