...
async function fetchGraphql<T>(
	...
	const result = await loadPage(context, GRAPHQL_ENDPOINT, {
		timeout,
...
async function renderRepo(
	...
	const data = await fetchGraphql<RepoQueryData>(context, REPO_QUERY, { name: repoName }, timeout, signal);
...
async function renderSearch(
	...
	const data = await fetchGraphql<SearchQueryData>(context, SEARCH_QUERY, { query }, timeout, signal);
...
export const handleSourcegraph: SpecialHandler = async (
	...
	try {
		...
		switch (target.type) {
			case "search": {
				const result = await renderSearch(context, target.query, timeout, signal);
				if (!result.ok) return null;
				return buildResult(result.content, { url, method: "sourcegraph-search", fetchedAt, notes });
			}
			case "file": {
				const rev = target.rev ?? "HEAD";
				const result = await renderFile(context, target.repoName, target.filePath, rev, timeout, signal);
				if (!result.ok) return null;
				return buildResult(result.content, { url, method: "sourcegraph-file", fetchedAt, notes });
			}
			case "repo": {
				const result = await renderRepo(context, target.repoName, timeout, signal);
				if (!result.ok) return null;
