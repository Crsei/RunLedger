...
import { combineSignals, tryParseJson, USER_AGENT } from "../internal/platform.ts";
import { ToolAbortError } from "../internal/abort.ts";
import type { RenderResult, ScraperContext, SpecialHandler } from "./types.ts";
import { buildResult, MAX_BYTES } from "./types.ts";
...
export const handleDocsRs: SpecialHandler = async (
	...
	try {
		...
		const response = await context.fetch(jsonUrl, {
