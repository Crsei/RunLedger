...
import { tryParseJson, USER_AGENT } from "../internal/platform.ts";
...
export const handleMusicBrainz: SpecialHandler = async (
	...
	try {
		...
		if (entity === "artist") {
			...
			const artist = await fetchJson<MusicBrainzArtist>(context, apiUrl, timeout, signal);
			...
			const release = await fetchJson<MusicBrainzRelease>(context, apiUrl, timeout, signal);
			if (!release) return null;
			md = buildReleaseMarkdown(release);
		} else {
			const apiUrl = `https://musicbrainz.org/ws/2/recording/${mbid}?fmt=json`;
			const recording = await fetchJson<MusicBrainzRecording>(context, apiUrl, timeout, signal);
