import { combineSignals, USER_AGENT } from "../internal/platform.ts";
import type { RenderResult, ScraperContext, SpecialHandler } from "./types.ts";
import { buildResult, formatMediaDuration, loadPage } from "./types.ts";
...
export async function fetchGitHubApi(
	...
	try {
		...
		const token = await context.credentials.getApiKey("github");
		if (token) {
			headers.Authorization = `Bearer ${token}`;
		}

		const response = await context.fetch(`https://api.github.com${endpoint}`, {
...
async function renderGitHubIssue(
	...
	const result = await fetchGitHubApi(context, endpoint, timeout, signal);
	...
	if (issue.comments > 0) {
		const comments = await fetchGitHubIssueComments(context, gh.owner, gh.repo, issue.number, issue.comments, timeout, signal);
...
async function renderGitHubCommit(
	...
	const result = await fetchGitHubApi(context, `/repos/${gh.owner}/${gh.repo}/commits/${gh.ref}`, timeout, signal);
	if (!result.ok || !result.data) return { content: "", ok: false };
...
async function renderGitHubIssuesList(
	...
	const result = await fetchGitHubApi(context, `/repos/${gh.owner}/${gh.repo}/issues?state=open&per_page=30`, timeout, signal);
...
async function renderGitHubTree(
	...
	const repoResult = await fetchGitHubApi(context, `/repos/${gh.owner}/${gh.repo}`, timeout, signal);
	...
	if (contentsResult.ok && Array.isArray(contentsResult.data)) {
		...
		if (readmeFile) {
			...
			const readmeResult = await loadPage(context, rawUrl, { timeout, signal });
			if (readmeResult.ok) {
...
async function renderGitHubRepo(
	...
	// Fetch README
	const readmeResult = await fetchGitHubApi(context, `/repos/${gh.owner}/${gh.repo}/readme`, timeout, signal);
...
async function fetchGitHubJobLogs(
	...
	const result = await loadPage(context, `https://api.github.com/repos/${owner}/${repo}/actions/jobs/${jobId}/logs`, {
		timeout,
		headers,
		signal,
	});
...
async function renderGitHubActionsRun(
	...
	const runResult = await fetchGitHubApi(context, `/repos/${gh.owner}/${gh.repo}/actions/runs/${gh.runId}`, timeout, signal);
...
async function renderGitHubActionsJob(
	...
	const jobResult = await fetchGitHubApi(context, `/repos/${gh.owner}/${gh.repo}/actions/jobs/${gh.jobId}`, timeout, signal);
	...
	const runResult = await fetchGitHubApi(context, `/repos/${gh.owner}/${gh.repo}/actions/runs/${job.run_id}`, timeout, signal);
	...
	const logs = await fetchGitHubJobLogs(context, gh.owner, gh.repo, job.id, timeout, signal);
	md += `## Logs\n\n`;
	md += logs
...
export const handleGitHub: SpecialHandler = async (
	...
	switch (gh.type) {
		case "blob": {
			...
			const result = await loadPage(context, rawUrl, { timeout, signal });
			if (result.ok) {
		...
		case "tree": {
			...
			const result = await renderGitHubTree(context, gh, timeout, signal);
			if (result.ok) {
		...
		case "commit": {
			notes.push(`Fetched via GitHub API`);
			const result = await renderGitHubCommit(context, gh, timeout, signal);
			if (result.ok) {
		...
		case "pull": {
			notes.push(`Fetched via GitHub API`);
			const result = await renderGitHubIssue(context, gh, timeout, signal);
			if (result.ok) {
				return buildResult(result.content, {
					url,
		...
		case "issues": {
			notes.push(`Fetched via GitHub API`);
			const result = await renderGitHubIssuesList(context, gh, timeout, signal);
			if (result.ok) {
		...
		case "repo": {
			notes.push(`Fetched via GitHub API`);
			const result = await renderGitHubRepo(context, gh, timeout, signal);
			if (result.ok) {
		...
		case "actions-run": {
			notes.push(`Fetched via GitHub API`);
			const result = await renderGitHubActionsRun(context, gh, timeout, signal);
			if (result.ok) {
		...
		case "actions-job": {
			notes.push(`Fetched via GitHub API`);
			const result = await renderGitHubActionsJob(context, gh, timeout, signal);
			if (result.ok) {
