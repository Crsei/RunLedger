...
export const handleLemmy: SpecialHandler = async (
	...
	try {
		...
		if (kind === "comment") {
			...
			const commentResult = await loadPage(context, commentUrl, { timeout, signal });
		...
		const [postResult, commentsResult] = await Promise.all([
			loadPage(context, postUrl, { timeout, signal }),
			loadPage(context, commentsUrl, { timeout, signal }),
		]);
