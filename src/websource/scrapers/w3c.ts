...
import { buildResult, htmlToBasicMarkdown, loadPage } from "./types.ts";
...
export const handleW3c: SpecialHandler = async (
	...
	try {
		...
		const [specResult, latestResult] = await Promise.all([
			loadPage(context, specUrl, { timeout, signal, headers: { Accept: "application/json" } }),
			loadPage(context, latestUrl, { timeout, signal, headers: { Accept: "application/json" } }),
		]);
		...
		let editors: string[] = [];
		if (editorsUrl) {
			const editorsResult = await loadPage(context, editorsUrl, { timeout: Math.min(timeout, 10), signal });
			if (editorsResult.ok) {
				try {
