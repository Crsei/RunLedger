...
import { buildResult, formatNumber, htmlToBasicMarkdown, loadPage } from "./types.ts";
 ...
 */
async function isMastodonInstance(
	...
	try {
		const result = await loadPage(context, `https://${hostname}/api/v1/instance`, {
...
export const handleMastodon: SpecialHandler = async (
	...
	try {
		...
		if (!(await isMastodonInstance(context, parsed.hostname, timeout, signal))) {
			return null;
		}
		...
		if (postMatch) {
			...
			const result = await loadPage(context, apiUrl, {
				timeout,
				headers: { Accept: "application/json" },
				signal,
			});
		...
		if (profileMatch) {
			...
			const result = await loadPage(context, lookupUrl, {
				timeout,
				headers: { Accept: "application/json" },
				signal,
			});
			...
			const statusesResult = await loadPage(context, statusesUrl, {
				timeout,
				headers: { Accept: "application/json" },
				signal,
			});
