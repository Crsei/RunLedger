...
import { buildResult, formatNumber, htmlToBasicMarkdown, loadPage, type ScraperContext, type SpecialHandler } from "./types.ts";

/**
 * Handle pub.dev URLs via API
 */
export const handlePubDev: SpecialHandler = async (url: string, timeout: number, context: ScraperContext, signal?: AbortSignal) => {
	try {
		...
		try {
			const readmeResult = await loadPage(context, readmeUrl, { timeout: Math.min(timeout, 10), signal });
			if (readmeResult.ok) {
