...
export const handleOpenLibrary: SpecialHandler = async (
	...
	try {
		...
		if (workMatch) {
			md = await fetchWork(context, workMatch[1], timeout, signal);
		} else if (editionMatch) {
			md = await fetchEdition(context, editionMatch[1], timeout, signal);
		} else if (isbnMatch) {
			md = await fetchByIsbn(context, isbnMatch[1], timeout, signal);
		}
...
async function fetchByIsbn(
	...
	let result = await loadPage(context, apiUrl, { timeout, signal });
	if (!result.ok) {
		result = await loadPage(context, apiUrl, { timeout, signal });
	}
	if (!result.ok) {
	...
	if (!book) {
		...
		const searchResult = await loadPage(context, searchUrl, { timeout, signal });
		if (!searchResult.ok) {
...
async function fetchAuthorNames(
	...
	const promises = authorKeys.slice(0, 5).map(async key => {
		...
		try {
			const result = await loadPage(context, apiUrl, { timeout: Math.min(timeout, 5), signal });
			if (result.ok) {
