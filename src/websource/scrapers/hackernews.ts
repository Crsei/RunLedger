import { tryParseJson } from "../internal/platform.ts";
import type { ScraperContext, SpecialHandler } from "./types.ts";
import { buildResult, decodeHtmlEntities, formatIsoDate, loadPage } from "./types.ts";
...
async function fetchItem(
	...
	const { content, ok } = await loadPage(context, url, { timeout, signal });
	if (!ok) return null;
...
async function fetchItems(
	...
	const promises = ids.slice(0, limit).map(id => fetchItem(context, id, timeout, signal));
...
async function renderStory(
	...
	if (item.kids && item.kids.length > 0 && depth < 2) {
		...
		const comments = await fetchItems(context, topComments, timeout, topComments.length, signal);
		...
		if (comments.length > 0) {
			...
			for (const comment of comments) {
				...
				if (comment.kids && comment.kids.length > 0 && depth < 1) {
					const childOutput = await renderStory(context, comment, timeout, depth + 1, signal);
					output += childOutput;
				}
			}
		}
...
async function renderListing(
	...
	let output = `# ${title}\n\n`;
	const stories = await fetchItems(context, ids, timeout, 20, signal);
...
export const handleHackerNews: SpecialHandler = async (
	url: string,
	timeout: number,
	context: ScraperContext,
	signal?: AbortSignal,
	...
	try {
		...
		if (itemId) {
			const item = await fetchItem(context, parseInt(itemId, 10), timeout, signal);
			if (!item) throw new Error(`Failed to fetch item ${itemId}`);

			content = await renderStory(context, item, timeout, 0, signal);
			notes.push(`Fetched HN item ${itemId} with top-level comments (depth 2)`);
		} else if (parsed.pathname === "/" || parsed.pathname === "/news") {
			const { content: raw, ok } = await loadPage(context, `${API_BASE}/topstories.json`, { timeout, signal });
			...
			content = await renderListing(context, ids, timeout, "Hacker News - Top Stories", signal);
			notes.push("Fetched top 20 stories from HN front page");
		} else if (parsed.pathname === "/newest") {
			const { content: raw, ok } = await loadPage(context, `${API_BASE}/newstories.json`, { timeout, signal });
			if (!ok) throw new Error("Failed to fetch new stories");
			const ids = tryParseJson<number[]>(raw);
			if (!ids) throw new Error("Failed to parse new stories");
			content = await renderListing(context, ids, timeout, "Hacker News - New Stories", signal);
			notes.push("Fetched top 20 new stories");
		} else if (parsed.pathname === "/best") {
			const { content: raw, ok } = await loadPage(context, `${API_BASE}/beststories.json`, { timeout, signal });
			if (!ok) throw new Error("Failed to fetch best stories");
			const ids = tryParseJson<number[]>(raw);
			if (!ids) throw new Error("Failed to parse best stories");
			content = await renderListing(context, ids, timeout, "Hacker News - Best Stories", signal);
