...
import type { LocalizedText, RenderResult, ScraperContext, SpecialHandler } from "./types.ts";
import { buildResult, formatNumber, getLocalizedText, htmlToBasicMarkdown, loadPage } from "./types.ts";
...
export const handleFirefoxAddons: SpecialHandler = async (
	...
	try {
		...
		const result = await loadPage(context, apiUrl, { timeout, headers: { Accept: "application/json" }, signal });
