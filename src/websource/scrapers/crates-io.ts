...
import { buildResult, formatNumber, loadPage, looksLikeHtml } from "./types.ts";
...
export const handleCratesIo: SpecialHandler = async (
	...
	try {
		...
		const readmeResult = await loadPage(context, docsRsUrl, { timeout: Math.min(timeout, 5), signal });
