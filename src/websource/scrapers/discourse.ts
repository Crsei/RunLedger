...
import { buildResult, formatIsoDate, htmlToBasicMarkdown, loadPage } from "./types.ts";
...
export const handleDiscourse: SpecialHandler = async (
	...
	try {
		...
		if (!topicId && postMatch) {
			const postResult = await loadPage(context, buildPostUrl(baseUrl, postMatch.postId), { timeout, signal });
		...
		const topicResult = await loadPage(context, buildTopicUrl(baseUrl, topicId), { timeout, signal });
