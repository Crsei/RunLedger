...
import { buildResult, formatNumber, loadPage, type RenderResult, type ScraperContext, type SpecialHandler } from "./types.ts";
...
export const handlePyPI: SpecialHandler = async (
	...
	try {
		...
		const [result, downloadsResult] = await Promise.all([
			...
			loadPage(context, downloadsUrl, { timeout: Math.min(timeout, 5), signal }),
		]);
