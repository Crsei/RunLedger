...
async function resolveHandle(
	...
	const result = await loadPage(context, url, {
		timeout,
	...
	try {
		...
		if (pathParts[0] === "profile" && pathParts[1]) {
			...
			if (pathParts[2] === "post" && pathParts[3]) {
				...
				const did = await resolveHandle(context, handle, timeout, signal);
				if (!did) return null;
				...
				const result = await loadPage(context, threadUrl, {
					timeout,
					headers: { Accept: "application/json" },
					signal,
				});
			...
			const result = await loadPage(context, profileUrl, {
				timeout,
				headers: { Accept: "application/json" },
				signal,
			});
