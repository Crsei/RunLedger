...
export const handleMDN: SpecialHandler = async (url: string, timeout: number, context: ScraperContext, signal?: AbortSignal) => {
	...
	try {
		const result = await loadPage(context, jsonUrl, { timeout, signal, headers: { Accept: "application/json" } });
