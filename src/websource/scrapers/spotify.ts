...
export const handleSpotify: SpecialHandler = async (url: string, timeout: number, context: ScraperContext, signal?: AbortSignal) => {
	...
	try {
		...
		const response = await loadPage(context, oEmbedUrl, { timeout, signal });
	...
	try {
		const pageResponse = await loadPage(context, url, { timeout, signal });
