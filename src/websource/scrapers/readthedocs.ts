...
import { buildResult, htmlToBasicMarkdown, loadPage, type RenderResult, type ScraperContext, type SpecialHandler } from "./types.ts";
...
export const handleReadTheDocs: SpecialHandler = async (
	...
	// Fetch the page
	const result = await loadPage(context, url, { timeout, signal });
	if (!result.ok) {
		return {
			url,
	...
	if (sourceUrl) {
		try {
			const sourceResult = await loadPage(context, sourceUrl, { timeout: Math.min(timeout, 10), signal });
