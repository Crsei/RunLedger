...
import { buildResult, formatNumber, loadPage } from "./types.ts";
...
export const handleHuggingFace: SpecialHandler = async (url: string, timeout: number, context: ScraperContext, signal?: AbortSignal) => {
		...
		switch (parsed.type) {
			case "model": {
				...
				const [apiResult, readmeResult] = await Promise.all([
					loadPage(context, apiUrl, { timeout, signal }),
					loadPage(context, readmeUrl, { timeout: Math.min(timeout, 5), signal }),
				]);
			...
			case "model_or_user": {
				...
				const modelResult = await loadPage(context, modelApiUrl, { timeout, signal });

				if (modelResult.ok) {
					const model = tryParseJson<HfModelData>(modelResult.content);
					if (model) {
						const readmeUrl = `https://huggingface.co/${parsed.id}/raw/main/README.md`;
						const readmeResult = await loadPage(context, readmeUrl, { timeout: Math.min(timeout, 5), signal });
				...
				const userResult = await loadPage(context, userApiUrl, { timeout, signal });
