 ...
 */
export const handleVimeo: SpecialHandler = async (url: string, timeout: number, context: ScraperContext, signal?: AbortSignal) => {
	try {
		const parsed = new URL(url);
		...
		const oembedResult = await loadPage(context, oembedUrl, { timeout, signal });
		...
		try {
			...
			const configResult = await loadPage(context, configUrl, { timeout: Math.min(timeout, 5), signal });
