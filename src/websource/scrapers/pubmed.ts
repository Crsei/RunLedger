...
import { buildResult, loadPage, type RenderResult, type ScraperContext, type SpecialHandler } from "./types.ts";
...
export const handlePubMed: SpecialHandler = async (
	...
	try {
		...
		const fetchWithRetry = async (requestUrl: string, acceptJson = true) => {
			let response = await loadPage(context, requestUrl, {
				timeout,
				signal,
				headers: {
					...NCBI_HEADERS,
			...
			if (!response.ok) {
				response = await loadPage(context, requestUrl, {
					timeout,
					signal,
					headers: {
						...NCBI_HEADERS,
		...
		try {
			...
			const meshResult = await loadPage(context, meshUrl, {
