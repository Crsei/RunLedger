...
export const handleSemanticScholar: SpecialHandler = async (url: string, timeout: number, context: ScraperContext, signal?: AbortSignal) => {
	...
	const { content, ok, finalUrl } = await loadPage(context, apiUrl, { timeout, signal });
