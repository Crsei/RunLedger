...
import { buildResult, formatIsoDate, formatNumber, loadPage } from "./types.ts";
...
export const handleNuGet: SpecialHandler = async (
	...
	try {
		...
		if (!latestPage.items && latestPage["@id"]) {
			const pageResult = await loadPage(context, latestPage["@id"], { timeout, signal });
		...
		if (requestedVersion) {
			...
			for (const page of index.items) {
				...
				if (!pageItems && page["@id"]) {
					const pageResult = await loadPage(context, page["@id"], { timeout: Math.min(timeout, 5), signal });
					if (pageResult.ok) {
		...
		const searchResult = await loadPage(context, searchUrl, { timeout: Math.min(timeout, 5), signal });
