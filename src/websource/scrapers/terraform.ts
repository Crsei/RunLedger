...
export const handleTerraform: SpecialHandler = async (
	...
	try {
		...
		if (moduleMatch) {
			...
			return await handleModuleUrl(context, url, namespace, name, provider, timeout, signal, fetchedAt);
		}
		...
		if (providerMatch) {
			...
			return await handleProviderUrl(context, url, namespace, type, timeout, signal, fetchedAt);
		}
