...
async function renderGitLabRepo(
	...
	// Try to fetch README
	if (repo.readme_url) {
		const readmeResult = await loadPage(context, repo.readme_url, { timeout, signal });
...
export const handleGitLab: SpecialHandler = async (
	...
	switch (gl.type) {
		case "blob": {
			const projectId = await getProjectId(context, gl, timeout, signal);
			if (!projectId) break;

			notes.push(`Fetched raw file via GitLab API`);
			const result = await renderGitLabFile(context, gl, projectId, timeout, signal);
			if (result.ok) {
		...
		case "tree": {
			...
			const result = await renderGitLabTree(context, gl, projectId, timeout, signal);
			if (result.ok) {
		...
		case "issue": {
			...
			const result = await renderGitLabIssue(context, gl, projectId, timeout, signal);
			if (result.ok) {
		...
		case "merge_request": {
			...
			const result = await renderGitLabMR(context, gl, projectId, timeout, signal);
			if (result.ok) {
		...
		case "repo": {
			...
			const result = await renderGitLabRepo(context, gl, timeout, signal);
			if (result.ok) {
