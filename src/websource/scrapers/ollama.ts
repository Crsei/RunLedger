...
import { buildResult, decodeHtmlEntities, loadPage } from "./types.ts";
...
export const handleOllama: SpecialHandler = async (
	...
	try {
		...
		if (shorthand) {
			const tagsResult = await loadPage(context, tagsUrl, tagsOptions);
			...
			const pageResult = await loadPage(context, pageUrl, { timeout, signal });
		...
		const [tagsResult, pageResult] = await Promise.all([
			loadPage(context, tagsUrl, tagsOptions),
			loadPage(context, pageUrl, { timeout, signal }),
		]);
