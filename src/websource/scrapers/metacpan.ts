...
import { buildResult, formatIsoDate, loadPage } from "./types.ts";
...
export const handleMetaCPAN: SpecialHandler = async (
	...
	try {
		...
		if (podMatch) {
			...
			return await fetchModule(context, url, moduleName, timeout, fetchedAt, signal);
		}
		...
		if (releaseMatch) {
			...
			return await fetchRelease(context, url, distribution, timeout, fetchedAt, signal);
		}
...
async function fetchModule(
	...
	const releaseResult = await loadPage(context, releaseUrl, { timeout: Math.min(timeout, 5), signal });
