...
export const handleDockerHub: SpecialHandler = async (
	...
	try {
		...
		const [repoResult, tagsResult] = await Promise.all([
			loadPage(context, repoUrl, { timeout, headers: { Accept: "application/json" }, signal }),
			loadPage(context, tagsUrl, { timeout: Math.min(timeout, 10), headers: { Accept: "application/json" }, signal }),
		]);
