...
class UnpackedAsarMemberSource implements MemberSource {
	...
	async read(size: number, memberPath: string): Promise<Uint8Array> {
		...
		// 上游这里用 `Bun.file` 读 unpacked member(相对 archive 的磁盘文件)。
		// RunLedger 不允许库层 raw-fs 旁路:unpacked member 必须由调用方经
		// governed fs 自行读取,这里显式拒绝而不是静默返回空内容。
		throw new ArchiveError(
			`Archive member '${label}' is unpacked; this port does not read files outside the archive`,
		);
	}
}
