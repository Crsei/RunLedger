import { Document, type DocumentFragment, Element, Text } from "./core.ts";
	...
	while (index < html.length) {
		...
		if (html[index + 1] === "/") {
			...
			const closingName = html
				...
				.split(/\s/, 1)[0]
				.toLowerCase();
			// `Array.prototype.findLastIndex` 需要 ES2023 lib,本仓库停在 ES2022,
			// 因此手写等价的反向查找。
			let matchIndex = -1;
			for (let candidate = stack.length - 1; candidate >= 0; candidate -= 1) {
				if (stack[candidate]?.localName === closingName) { matchIndex = candidate; break; }
			}
