...
async function callZaiSearch(apiKey: string, params: ZaiSearchParams): Promise<unknown> {
	...
	for (let i = 0; i < attempts.length; i++) {
		try {
			return await callZaiTool(apiKey, attempts[i], params.signal, params.fetch, params.timeoutMs);
		} catch (error) {
			lastError = error;
...
export async function searchZai(params: ZaiSearchParams): Promise<SearchResponse> {
	const rawResult = await withApiKey(
		params.credentials,
		"zai",
		"Z.AI credentials not found. Set ZAI_API_KEY or login with 'omp /login zai'.",
		key => callZaiSearch(key, params),
	);
