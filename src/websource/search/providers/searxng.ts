 ...
 *   searxng.categories    - Optional categories filter (list of names)
 *   searxng.engines       - Optional engine names or shortcuts
 *                           (e.g. ["duckduckgo", "br", "sp"]); shortcuts resolve
 *                           via the instance's /config endpoint
 *   searxng.language      - Optional language code (e.g. en, zh-CN)
 *
 * The composition layer resolves the environment-variable fallbacks below into
 * the injected settings snapshot; this module never reads the process
 * environment itself.
...
import type { SearxngSettings, WebSearchSettings } from "../../settings.ts";
...
import { formatScraperQuery, parseSearchQuery } from "../query.ts";
...
/** SearXNG's `engines=` / `categories=` parameters take comma-separated lists. */
function joinSetting(values: readonly string[] | undefined): string | undefined {
...
/** Find SearXNG authentication from the injected settings snapshot. Basic auth takes precedence over bearer tokens. */
function findAuth(searxng: SearxngSettings | undefined): SearXNGAuth | null {
	const basicUsername = searxng?.basicUsername ?? null;
	const basicPassword = searxng?.basicPassword ?? null;
...
async function callSearXNGSearch(
	...
	const response = await params.fetch(url.href, {
		headers,
...
export async function searchSearXNG(params: {
	...
	const searxng = params.settings?.searxng;
	const endpoint = searxng?.endpoint;
	if (!endpoint) {
		throw new Error(
	...
	const auth = findAuth(searxng);

	const categories = joinSetting(searxng?.categories);
	let language = searxng?.language;
	const configuredSafesearch = searxng?.safesearch;
	if (
	...
	const configuredEngines = joinSetting(searxng?.engines);
...
export class SearXNGProvider extends SearchProvider {
	...
	isAvailable(_credentials: WebSearchCredentialPort, settings?: WebSearchSettings): boolean {
		return Boolean(settings?.searxng?.endpoint);
	}
