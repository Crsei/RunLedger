import type { WebSearchCredentialId, WebSearchCredentialPort } from "../../credentials.ts";
...
/**
 * 解析一个检索凭据:由注入的 port 统一处理「存储值优先、白名单环境变量兜底」。
 * 上游对应物是 `findCredential(storage, envKey, ...providers)`。
 */
export async function findCredential(
	credentials: WebSearchCredentialPort,
	id: WebSearchCredentialId,
): Promise<string | null> {
	const key = await credentials.getApiKey(id);
 ...
 * outbound request is guaranteed to settle within `ms` even when the
