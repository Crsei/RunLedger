...
async function callTavilySearch(
	...
	if (fetchImpl === undefined) throw new Error("tavily: governed transport is not injected");
	const response = await fetchImpl(TAVILY_SEARCH_URL, {
		...
		signal: withHardTimeout(signal, params.timeoutMs),
	});
...
export async function searchTavily(params: SearchParams): Promise<SearchResponse> {
	...
	const callWithAuth = (searchParams: TavilySearchParams) =>
		withApiKey(
			...
			key => callTavilySearch(key, searchParams, params.signal, params.fetch),
		);
