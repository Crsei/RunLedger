...
import { deferred, sleep } from "../../internal/platform.ts";
import { formatSearchProviderFailures, getSearchProvider } from "../provider.ts";
...
export async function searchPublicWeb(
	...
	const excluded = new Set(params.settings?.exclude ?? []);
	const engineIds = PUBLIC_ENGINE_IDS.filter(id => !excluded.has(id));
	...
	if (!responses.some(response => response !== undefined) && failures.length < engineIds.length) {
		await Promise.race([all, firstSuccess.promise, sleep(Math.max(0, hardMs - softMs))]);
	}
	straggler.abort();
