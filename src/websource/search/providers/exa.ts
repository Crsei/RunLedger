 ...
 */
import { deferred, isRecord } from "../../internal/platform.ts";
...
import { isSearchResponse } from "../../exa/mcp-client.ts";
import { readMcpJsonRpcResponse } from "../../internal/mcp-rpc.ts";
...
/** 上游 `getDefault("exa.searchDelayMs")` 的 schema 默认值。 */
const DEFAULT_EXA_SEARCH_DELAY_MS = 1_000;

let nextExaSearchRequestAt = 0;
let exaSearchThrottle = Promise.resolve();

/** 注入的设置快照；未提供 `exa.searchDelayMs` 时按上游 schema 默认 1000ms 计。 */
function configuredExaSearchDelayMs(settings: WebSearchSettings | undefined): number {
	const delayMs = settings?.exa?.searchDelayMs ?? DEFAULT_EXA_SEARCH_DELAY_MS;
...
function waitUntilDoneOrAborted<T>(promise: Promise<T>, signal: AbortSignal | undefined): Promise<T> {
	...
	signal.throwIfAborted();
	const { promise: aborted, reject } = deferred<never>();
...
async function waitForExaSearchSlot(delayMs: number, signal: AbortSignal | undefined): Promise<void> {
	if (delayMs <= 0) return;
...
export interface ExaSearchParams {
	...
	/** 凭据唯一来源 (`exa` 的 API key 或空);由 provider 的 search() 注入。 */
	credentials: WebSearchCredentialPort;
	/** web 检索设置快照 (`exa.enabled` / `exa.searchDelayMs`)。 */
...
async function callExaSearch(apiKey: string, params: ExaSearchParams): Promise<ExaSearchResponse> {
	...
	await waitForExaSearchSlot(configuredExaSearchDelayMs(params.settings), params.signal);
	const response = await params.fetch(EXA_API_URL, {
		method: "POST",
		headers: {
...
async function callExaMcpSearch(params: ExaSearchParams): Promise<ExaSearchResponse> {
	...
	const apiKey = await params.credentials.getApiKey("exa");
	...
	const response = await params.fetch(`${EXA_MCP_URL}?${query.toString()}`, {
		method: "POST",
		headers: {
			"Content-Type": "application/json",
...
export async function searchExa(params: ExaSearchParams): Promise<SearchResponse> {
	// 凭据 port 同时覆盖存储值与白名单环境变量;解析到 key 时走 REST,
	// 否则回落 Exa 的公开 MCP(keyless)。
	const apiKey = await params.credentials.getApiKey("exa");
...
export class ExaProvider extends SearchProvider {
	...
	isAvailable(credentials: WebSearchCredentialPort, settings?: WebSearchSettings): Promise<boolean> | boolean {
		if (!this.#settingsAllowSearch(settings)) return false;
		return credentials.has("exa");
	}
	 ...
	 */
	override isExplicitlyAvailable(
		_credentials: WebSearchCredentialPort,
		settings?: WebSearchSettings,
	): boolean {
		return this.#settingsAllowSearch(settings);
	}

	#settingsAllowSearch(settings: WebSearchSettings | undefined): boolean {
		return settings?.exa?.enabled !== false;
	}
