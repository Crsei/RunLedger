...
/** 根据配置的字母计算纯文字 wordmark 的可见宽度。 */
export function logoLineWidth(value?: string): number {
	return visibleWidth(normalizeLogoLetters(value));
}
...
/**
 * 按字母渲染清晰的纯文字 Logo：前 3 个字母使用 dim，其余字母使用 bold。
 * 旧块字形仍保留为兼容数据，但不再混入 Welcome 页面，避免 Logo 文案不可辨认。
 */
export function renderLogo(theme: Theme, value?: string): string[] {
	...
	const wordmark = glyphs
		.map((glyph, glyphIndex) => paint(glyph.letter, glyphIndex < 3 ? dim : bright))
		.join("");
	return [wordmark];
}
