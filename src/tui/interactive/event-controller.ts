		...
		try {
			switch (ev.type) {
				...
				case "agent_end":
					{
						...
						this.port.refs.status.setStopReason(stopReason);
				...
				case "agent_work_resume":
					...
					break;
				case "turn_start":
					this.port.refs.status.setTurn(ev.turn);
					break;
				case "turn_end":
					this.port.refs.status.setTurn(ev.turn);
					if (ev.stopReason) this.port.refs.status.setStopReason(ev.stopReason);
					break;
				case "message_start":
				...
				case "queue_update":
					this.port.refs.status.setQueueCounts(ev.steering.length, ev.followUp.length);
					break;
			}
		} catch (e) {
