/** 分层配置解析为 canonical SecuritySnapshot。 */
...
function collectPaths(
	...
	const values = layers.flatMap((layer) => layer.document.filesystem?.[key] ?? []);
...
export function resolveSecuritySnapshot(options: ResolveSecuritySnapshotOptions): SecurityResult<SecuritySnapshot> {
	...
	if (constraints) {
		...
		if (!constraints.allowedApprovalPolicies.includes(approvalPolicy)) {
			return failure(`approval policy is forbidden by managed policy: ${approvalPolicy}`);
		}
		if (!minimumSandboxSatisfied(sandbox, constraints.minimumSandbox)) {
			return failure(`sandbox profile is weaker than managed minimum: ${sandbox}`);
		}
	...
	const filesystem: FilesystemPolicy = {
		...
		writeRoots: defaults.filesystemMode === "read-only"
			? [options.tempRoot]
			: writeRoots.value.length > 0 ? writeRoots.value : [options.workspaceRoot, options.tempRoot],
		denyRead: denyRead.value,
		denyWrite: denyWrite.value,
		protectedPaths: [...new Set([
			resolve(options.workspaceRoot, ".git"),
			resolve(options.workspaceRoot, ".runledger"),
			...protectedPaths.value,
		])].sort(),
	};
	const rules: SecurityRule[] = options.layers.flatMap((layer) =>
		(layer.document.rules ?? []).map((rule) => ({ ...rule, source: layer.source })),
	);
	...
	return { ok: true, value: { ...body, policyDigest: canonicalDigest(body) } };
}
