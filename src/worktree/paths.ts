...
import type { RepositoryId, WorkspaceId } from "../runtime/contracts/public.ts";
...
export function validateWorktreeLabel(label: string): WorktreeResult<string> {
	if (!/^[a-z0-9][a-z0-9-]{0,62}$/u.test(label) || label.endsWith("-") || label.includes("--")) return failure("worktree label must be a lowercase slug without path syntax");
...
export function buildManagedWorktreePath(
	...
	const target = resolve(managedRoot, idTail(repositoryId), `${valid.value}-${idTail(workspaceId)}`);
...
export function validateBranchName(branch: string): WorktreeResult<string> {
	if (
		branch.length === 0 || branch.length > 240 || branch.startsWith("-") || branch.startsWith(".") || branch.endsWith(".") || branch.endsWith("/") || branch.endsWith(".lock") ||
