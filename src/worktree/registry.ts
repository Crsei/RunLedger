...
import { isRuntimeId, type RuntimeInstanceId, type SessionId, type WorkspaceId } from "../runtime/contracts/public.ts";
...
export { NodeJsonlWorktreeRegistryStore as JsonlWorktreeRegistryStore } from "../storage/worktree-registry-store.ts";
...
export class WorktreeRegistry {
	...
	async #readState(): Promise<WorktreeResult<RegistryState>> {
		try {
			...
			return failure("registry_failed", error instanceof Error ? `worktree registry is unavailable: ${error.message}` : "worktree registry is unavailable", true);
		}
	}
	...
	public async transact<T>(operation: (transaction: RegistryTransaction) => Promise<WorktreeResult<T>> | WorktreeResult<T>): Promise<WorktreeResult<T>> {
		try {
			...
			return failure("registry_failed", error instanceof Error ? `worktree registry transaction failed: ${error.message}` : "worktree registry transaction failed", true);
		}
	}
